...
public class LoginActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mProgressViewRelative = findViewById(R.id.login_progress_relative);
    }
...
//            mAuthTask.execute((Void) null);
    ...
    private void loginToRemoteServer(String username, String password) {
        ...
        GsonRequest<UserObject> serverRequest = new GsonRequest<UserObject>(
                ...
                UserObject.class,
                params,
                createRequestSuccessListener(username),
                createRequestErrorListener());

        serverRequest.setRetryPolicy(new DefaultRetryPolicy(5*DefaultRetryPolicy.DEFAULT_TIMEOUT_MS, 0, 0));


        VolleySingleton.getInstance(LoginActivity.this).addToRequestQueue(serverRequest);
    }

    private Response.Listener<UserObject> createRequestSuccessListener(final String username) {
...
//                        if (!TextUtils.isEmpty(response.getStatus()) && response.getStatus().equals("inWork")) {
                            //save user login data to a shared preference
//                        if(response.getEmployeeTypeName().equals("Machine Operator")) {
                            response.setRepName(username);
                            ...
                            Helper.displayErrorMessage(LoginActivity.this, "Successfull", "success");
                            Intent loginUserIntent = new Intent(LoginActivity.this, LoginActivity.class);
                            loginUserIntent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_NEW_TASK);
                            startActivity(loginUserIntent);
                            finish();
//                        }else if(response.getEmployeeTypeName().equals("Engineer")){}
//                        } else {
//                            showProgress(false);
//                            Helper.displayErrorMessage(LoginActivity.this, "You are out of work now", "error");
//                        }
                    } else {
                        showProgress(false);
                        Helper.displayErrorMessage(LoginActivity.this, "Invalid username or password", "error");
                    }
                } catch (Exception e) {
                    showProgress(false);
                    Helper.displayErrorMessage(getApplicationContext(), "Something went wrong", "error");
                    e.printStackTrace();
                }
            }
        };
    ...
    private Response.ErrorListener createRequestErrorListener() {
        return new Response.ErrorListener() {
            ...
            public void onErrorResponse(VolleyError error) {
                showProgress(false);
                Helper.displayErrorMessage(LoginActivity.this, "Something went wrong!", "error");
//                hideProgressDialog();
